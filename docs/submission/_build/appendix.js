...
function build() {
  ...
  c.push(P([["Казахский — отдельная проверка. ", { bold: true }], "Русские записи ничего не говорят о казахском, поэтому взят публичный эталон FLEURS (kk_kz, CC-BY-4.0): 40 фраз с проверенными расшифровками. Для сравнения добавлена специализированная казахская модель shyngys879/kazakh-whisper-large-v3-turbo (дообучена на 841 тыс. примеров, включая ISSAI KSC2), переведённая в CTranslate2 int8."]));
  c.push(...table(["Модель (казахский, FLEURS)", "Ошибок по словам", "По символам", "Время на фразу"], [
    ["Whisper small", "72.7%", "21.6%", "5,8 с"],
    ["NVIDIA FastConformer kk+ru", "10.9%", "4.1%", "1,5 с"],
    ["Казахский Whisper turbo", "10.2%", "2.8%", "22,2 с"],
  ], [2.8, 1.6, 1.2, 1.4], { boldFirst: true }));
  c.push(P("Специализированная казахская модель точнее двуязычной лишь на 0,7 пункта, но медленнее в 15 раз: для живого режима она непригодна и остаётся необязательным движком для разбора записей. FLEURS — читаная студийная речь; на телефонном звуке ошибок больше, эти числа сравнивают модели между собой."));
  ...
  for (const t of [
    ...
    "ASR остаётся узким местом: переход на двуязычную модель снял худшее (казахский — 10.9% ошибок по словам против 72.7% у Whisper small), но измерено это на студийной речи FLEURS; на телефонном звуке ошибок больше.",
