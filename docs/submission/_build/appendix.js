...
function build() {
  ...
  c.push(H2("4.4. Живой режим: задержка"));
  c.push(P("Конвейер demo/live.py: звук (микрофон или запись) → Silero VAD режет поток на реплики по паузам → faster-whisper → детектор. Замер на записи реального звонка длиной 91 с, процессор ноутбука Intel Core Ultra 7 255U, Whisper small, язык задан (results/v3/live_latency_small_ru.json)."));
  c.push(...table(["Звено", "Медиана"], [
    ["Ожидание паузы (конец фразы)", "0,6 с"],
    ["Распознавание речи", "2,4 с"],
    ["Детектор (префикс до 256 токенов)", "0,26 с"],
    ["Итого: от конца фразы до риска на экране", "3,3 с (90% реплик — до 3,9 с)"],
  ], [3.4, 3.4], { boldFirst: true }));
  c.push(P("Очередь не копится: 15 реплик за 91 секунду обработаны вовремя. Автоопределение языка удваивает задержку (кодировщик Whisper прогоняется дважды), а large-v3-turbo на этом процессоре тратит 11 с на фразу — для крупных моделей нужна видеокарта. Три секунды несущественны: тревога обычно звучит за несколько реплик до просьбы о коде, то есть за десятки секунд."));
