...
Две страницы:
  * «Разговор по репликам» — примеры из отложенного набора и свой разговор,
    реплика за репликой; после каждой реплики модель (XLM-RoBERTa + LoRA)
    пересчитывает риск по всему сказанному, рядом — rule-baseline на красных флагах;
  * «Живой звонок» (live_page.py) — звук с микрофона или из файла, распознавание
    речи и риск в реальном времени.
...
from common import (C_MODEL, D, MODEL_DIR, RULE_TH, load, plural, redflags,  # noqa: E402,F401
                    risk_chart, score_last)
...
pg = st.navigation([
    st.Page(dialogue_page, title="Разговор по репликам", icon="💬", default=True),
    st.Page("live_page.py", title="Живой звонок", icon="🎙️", url_path="live"),
])
pg.run()
