...
Пять проверок:
  ...
  5. Слова доверия и вежливости (v3.1) — «официальный», «спасибо», «хорошо»,
     «всего доброго» и казахские аналоги в репликах звонящего должны
     встречаться в обоих классах: мошенники говорят так же вежливо. В v3 они
     были только у легитимных звонков, и модель пропускала «официальную
     программу поддержки».
"""
...
MARKER_MIN_SHARE = 0.03        # доля меньшинства ниже — слово доверия выдаёт класс
TRUST_MARKERS = {
    "официальный": r"официальн", "ресми": r"ресми", "спасибо": r"спасибо", "рахмет": r"рахмет",
    "хорошо": r"хорошо", "жақсы": r"жақсы", "всего доброго": r"всего доброго",
    "сау болыңыз": r"сау болыңыз", "сайт": r"сайт", "документ/құжат": r"документ|құжат",
    "пожалуйста/өтінемін": r"пожалуйста|өтінемін",
}
...
def check_markers(D):
    """Доля диалогов, где слово звучит у звонящего, — по классам."""
    import re
    bad = []
    print(f"{'слово':22s} {'scam':>6s} {'benign':>7s} {'меньшинство':>12s}")
    for name, pat in TRUST_MARKERS.items():
        c = Counter(d["label"] for d in D
                    if any(re.search(pat, t["text"], re.I) for t in d["turns"] if t["speaker"] == "caller"))
        s, b = c["scam"], c["benign"]
        share = min(s, b) / (s + b) if s + b else 0
        flag = ""
        if s + b >= 30 and share < MARKER_MIN_SHARE:
            bad.append(name)
            flag = "  ← только в одном классе"
        print(f"{name:22s} {s:6d} {b:7d} {share:11.1%}{flag}")
    return bad
...
def main():
    ...
    print("\n" + "═" * 78 + "\n5. СЛОВА ДОВЕРИЯ И ВЕЖЛИВОСТИ у звонящего (train)\n" + "═" * 78)
    bad_markers = check_markers(train)
    ...
    print(f"  слов доверия только в одном классе: {len(bad_markers)}" +
          (f"  {bad_markers}" if bad_markers else ""))
